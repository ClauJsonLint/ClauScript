...
namespace wiz {
	...
	public:
		std::string id;
		wiz::ArrayMap<std::string, DataType> local;
		wiz::ArrayMap<std::string, DataType> parameter;
	...
	std::map<std::string, Event> Make(wiz::load_data::UserType* _global);

	class Expression { public: virtual DataType run(Event* e) = 0; };

	class If : public Expression {
	public:
		std::unique_ptr<Expression> cond;
		std::vector<std::unique_ptr<Expression>> statements; /// if(cond->value) { statement.... }
	
	public:
		virtual DataType run(Event* e) {
			if (cond->run(e).ToInt()) {
				for (int i = 0; i < statements.size(); ++i) {
					statements[i]->run(e);
					if (e->return_value != "") { // use_...
						return DataType();
					}
				}
			}return DataType();
	...
	class AssignLocal : public Expression {
	public:
		std::string var;
	...
	public:
		virtual DataType run(Event* e) {
			e->local[var] = val->run(e);
			return DataType();
		}
	};

	class GetParameter : public Expression {
	public:
		std::string param;
	...
	public:
		virtual DataType run(Event* e) {
			return e->local[local_name];
		}
	};

	class SetLocal : public Expression {
	public:
		std::vector<DataType> local_name;
	public:
		virtual DataType run(Event* e) {
		//	for (auto& x : local_name) {
		//		e->local[x.ToString()] = DataType();
		//	}
			return DataType();
		}
	};

	class SetParameter : public Expression {
	public:
		std::vector<DataType> param_name;
	public:
		virtual DataType run(Event* e) {
		//	for (auto& x : param_name) {
		//		e->parameter[x.ToString()] = DataType();
		//	}
			return DataType();
		}
	};

	class GetTrue : public Expression {
	public:
		virtual DataType run(Event* e) {
			DataType x;
			x.SetInt(1);
	...
	class GetFalse : public Expression {
	public:
		virtual DataType run(Event* e) {
			DataType x;
			x.SetInt(0);
	...
	class GetString : public Expression {
	public:
		std::string str;
	public:
		virtual DataType run(Event* e) {
	...
	public:
		virtual DataType run(Event* e) {
			...
			auto result = wiz::load_data::Utility::Compare(left->run(e).ToString(), right->run(e).ToString());
	...
	class IsNot : public Expression {
	public:
std::unique_ptr<Expression> expr;
	public:
		virtual DataType run(Event* e) {
			DataType x;
			x.SetInt(!(expr->run(e).ToInt()));
			return x;
		}
	};

	class IsAndAll : public Expression {
	public:
	public:
	...
	class IsCOMPBig : public Expression { // >
	public:
	public:
		std::unique_ptr<Expression> left;
		std::unique_ptr<Expression> right;
	public:
		virtual DataType run(Event* e) {
			...
			auto result = wiz::load_data::Utility::Compare(a.ToString(), b.ToString());
			//std::cout <<"chk " <<  a.ToString() << " " << b.ToString() << " " << result << "\n";
			if (result == "> 0"sv) {
				x.SetInt(1);
			}
			else {
	...
	class IsQuoted : public Expression {
	public:
		std::unique_ptr<Expression> expr;
	public:
		virtual DataType run(Event* e) {
			DataType x;
			std::string str = expr->run(e).ToString();
			x.SetInt(str.size() >= 2 && str.back() == str.front() && str.back() == '\"');
			return x;
		}
	};

	class RemoveQuoted : public Expression {
	public:
		std::unique_ptr<Expression> expr;
	public:
		virtual DataType run(Event* e) {
			std::string x = expr->run(e).ToString();
			if (x.size() >= 2) {
	...
	std::unique_ptr<Expression> _Make(wiz::load_data::UserType* ut);
	std::unique_ptr<Expression> _Make(const wiz::load_data::ItemType<DataType>& item);
	std::unique_ptr<Expression> _Make2(const wiz::load_data::ItemType<DataType>& item);
			...
			for (int i = 0; i < ut->GetIListSize(); ++i) {
				if (ut->IsItemList(i)) {
					if (!ut->GetItemList(it_count).GetName().empty()) {
						result += " ";
						result += _Make2(ut->GetItemList(it_count))->run(e).ToString();
						result += " = ";
					}
					if (!ut->GetItemList(it_count).Get().empty()) {
						result += _Make(ut->GetItemList(it_count))->run(e).ToString();
					}
	...
	public:
		...
		bool operator<(const SortInfo& info) const
		{
			...
			if (temp == "< 0"sv) { return true; }
			else if (temp == "> 0"sv) { return false; }
			else if (temp == "== 0"sv) {
				return idx < info.idx;
			}
			else {
	...
	public:
		...
		bool operator<(const SortInfo2& info) const
		{
			...
			std::string_view temp = wiz::load_data::Utility::Compare(this->data, info.data);
			if (temp == "< 0"sv) { return false; }
			else if (temp == "> 0"sv) { return true; }
			else if (temp == "== 0"sv) {
				return idx < info.idx;
			}
			else {
...
namespace wiz {
	...
	class ClauText2 : public wiz::load_data::EXCUTE_MODULE_BASE {
	public:
		ClauText2(const ClauText2&) = delete;
		ClauText2& operator=(const ClauText2&) = delete;
	...
	namespace load_data {
		...
		public:
			...
			*/
			...
			static void _NewIterate(UserType& global, const std::string& dir, const std::vector<wiz::load_data::UserType*>& ut,
				std::map<std::string, Event>& event_table, const std::vector<std::string>& event_name, const std::string& recursive)
			{
				for (int i = 0; i < ut.size(); ++i) {

					for (int j = 0; j < ut[i]->GetItemListSize(); ++j) {
						for (int k = 0; k < event_name.size(); ++k)
						{
							std::string id = event_name[k];
							Event& e = event_table[id];

							e.global = &global;

							e.parameter["name"] = ut[i]->GetItemList(j).GetName();
							e.parameter["value"] = ut[i]->GetItemList(j).Get();
							e.parameter["is_user_type"] = "0";

							for (int iter = 0; iter < e.statements.size(); ++iter) {
								e.statements[iter]->run(&e);
							}

							std::string result = e.return_value.ToString();
							e.return_value = "";
							...
							if (result.empty() == false) {
								...
								auto name = resultUT.GetItem("name");
								if (name.empty() == false) {
									ut[i]->GetItemList(j).SetName(ToString(name[0].Get(0)));
								}

								auto value = resultUT.GetItem("value");
								if (value.empty() == false) {
									ut[i]->GetItemList(j).Set(0, value[0].Get(0));
								}
							}
					...
					for (int j = 0; j < ut[i]->GetUserTypeListSize(); ++j) {
						...
						for (int k = 0; k < event_name.size(); ++k)
						{
							...
							e.parameter["name"] = ut[i]->GetUserTypeList(j)->GetName();
							e.parameter["value"] = "NONE";
							e.parameter["is_user_type"] = "1";
							...
							if (result.empty() == false) {
								...
								auto name = resultUT.GetItem("name");
								if (name.empty() == false) {
									ut[i]->GetUserTypeList(j)->SetName(ToString(name[0].Get(0)));
								}
							}

							//// recursive
							if ("TRUE" == recursive) {
								const std::string name = wiz::ToString(ut[i]->GetUserTypeList(j)->GetName());
								_NewIterate(global, dir + (name.empty() ? "_" : name) + "/",
									std::vector<UserType*>{ ut[i]->GetUserTypeList(j) }, event_table, event_name, recursive);
							}
						}
					}
				}
...
//}
				...
				***/
				auto tree = Make(&eventsTemp);

				_NewIterate(global, dir, ut, tree, events, recursive);
