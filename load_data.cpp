...
namespace wiz {
	namespace load_data {
			...
			wiz::ArrayStack<WIZ_STRING_TYPE>& operandStack, const ExecuteData& excuteData)
		{
			...
			// todo - Is~ ?? others ??
				...
				operandStack.push(std::string(wiz::load_data::Utility::GetType(operandStack.pop().ToString())));
			}
	...
	{
		std::string name = item.GetName().ToString();

		DataType value = item.Get();

		if (!value.empty()) {
			if (wiz::String::startsWith(value.ToString(), "$parameter.")) {
				auto x = std::make_unique<GetParameter>();
				x->param = value.ToString().substr(11);
				return x;
			}
			else if (wiz::String::startsWith(value.ToString(), "$local.")) {
				auto x = std::make_unique<GetLocal>();
				x->local_name = value.ToString().substr(7);
				return x;
			}
			else  if (value.ToString() == "TRUE"sv) {
				auto x = std::make_unique<GetTrue>();
				return x;
			}
			else if (value.ToString() == "FALSE"sv) {
				auto x = std::make_unique<GetFalse>();
				return x;
			}
			else {
				auto x = std::make_unique<GetString>();
				x->str = value.ToString();
				return x;
			}
		}
		else {
		...
		throw "Error in _Make";
	}

	std::unique_ptr<Expression> _Make2(const wiz::load_data::ItemType<DataType>& item)
	{
		DataType name = item.GetName();

		if (!name.empty()) {
			if (wiz::String::startsWith(name.ToString(), "$parameter.")) {
				auto x = std::make_unique<GetParameter>();
				x->param = name.ToString().substr(11);
				return x;
			}
			else if (wiz::String::startsWith(name.ToString(), "$local.")) {
				auto x = std::make_unique<GetLocal>();
				x->local_name = name.ToString().substr(7);
				return x;
			}
			else  if (name.ToString() == "TRUE"sv) {
				auto x = std::make_unique<GetTrue>();
				return x;
			}
			else if (name.ToString() == "FALSE"sv) {
				auto x = std::make_unique<GetFalse>();
				return x;
			}
			else {
	...
	{
		std::string name = ut->GetName().ToString();

		{
			if (name == "$if"sv) {
				auto x = std::make_unique<If>();
				x->cond = _Make(ut->GetUserTypeList(0)->GetUserTypeList(0));
				for (int i = 0; i < ut->GetUserTypeList(1)->GetUserTypeListSize(); ++i) {
					x->statements.push_back(_Make(ut->GetUserTypeList(1)->GetUserTypeList(i)));
				}
				return std::move(x);
			}
			else if (name == "$assign"sv) { // todo - assign_local or assign_parameter or assign_global.
				auto x = std::make_unique<AssignLocal>();

				x->var = ut->GetItemList(0).Get().ToString(); // $local.
				x->var = x->var.substr(7);
				x->val = _Make(ut->GetUserTypeList(0));

				return std::move(x);
			}
			else if (name == "$AND_ALL"sv) {
				auto x = std::make_unique<IsAndAll>();
				int it_count = 0;
				int ut_count = 0;
				for (int i = 0; i < ut->GetIListSize(); ++i) {
					if (ut->IsItemList(i)) {
						x->exprs.push_back(_Make(ut->GetItemList(it_count)));
						it_count++;
					}
					else {
						x->exprs.push_back(_Make(ut->GetUserTypeList(ut_count)));
						ut_count++;
					}
				}
				...
				auto x = std::make_unique<IsEqual>();
				int it_count = 0;
				int ut_count = 0;
				for (int i = 0; i < ut->GetIListSize(); ++i) {
					if (ut->IsItemList(i)) {
						if (i == 0) {
							x->left = (_Make(ut->GetItemList(it_count)));
						}
						else {
							x->right = _Make(ut->GetItemList(it_count));
						}
						it_count++;
					}
					else {
						if (i == 0) {
							x->left = (_Make(ut->GetUserTypeList(ut_count)));
						}
						else {
							x->right = _Make(ut->GetUserTypeList(ut_count));
						}
						ut_count++;
					}
				}
				...
				auto x = std::make_unique<IsCOMPBig>();
				int it_count = 0;
				int ut_count = 0;
				for (int i = 0; i < ut->GetIListSize(); ++i) {
					if (ut->IsItemList(i)) {
				...
				auto x = std::make_unique<IsNot>();
				if (ut->GetItemListSize() > 0) {
					x->expr = _Make(ut->GetItemList(0));
				}
				else {
					x->expr = _Make(ut->GetUserTypeList(0));
				}

				return std::move(x);
			}
			else if (name == "$is_quoted_str"sv) {
				auto x = std::make_unique<IsQuoted>();
				if (ut->GetItemListSize() > 0) {
					x->expr = _Make(ut->GetItemList(0));
				}
				else {
				...
				auto x = std::make_unique<RemoveQuoted>();
				if (ut->GetItemListSize() > 0) {
					x->expr = _Make(ut->GetItemList(0));
				}
				else {
				...
				auto x = std::make_unique<SetLocal>();

				for (int i = 0; i < ut->GetItemListSize(); ++i) {
					x->local_name.push_back(ut->GetItemList(i).Get());
				}

				return std::move(x);
			}
			else if (name == "$parameter"sv) {
				auto x = std::make_unique<SetParameter>();

				for (int i = 0; i < ut->GetItemListSize(); ++i) {
					x->param_name.push_back(ut->GetItemList(i).Get());
				}
				return std::move(x);
			}
			else if (name == "$return"sv) {
				auto x = std::make_unique<ReturnStatement>();
	...
	{		
		std::map<std::string, Event> event_table;
		std::vector<wiz::load_data::UserType*> events = _global->GetUserTypeItem("Event");

		for (int i = 0; i < events.size(); ++i) {
			std::string id = events[i]->GetItem("id").at(0).Get().ToString();
			Event e;

			e.id = id;
			for (int j = 0; j < events[i]->GetUserTypeListSize(); ++j) {
				e.statements.push_back(_Make(events[i]->GetUserTypeList(j)));
			}
			
			event_table.insert(std::make_pair(id, std::move(e)));
		}

		return event_table; // change 'return type'?
	}
}